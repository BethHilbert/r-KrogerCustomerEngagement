...
* Display_desc represents in-store promotion location. For display I created factor levels with the assumption that End Caps are better than aisles, and front of store is better than rear.
...
#### Aggregating to the Product Grain

I looked at the data through the lens of product. I arranged the data so that one product is a single row in the dataframe. I summarized the number of times over the two year period this product was part of specific displays, calculated quantities, number of baskets, dollars, stores.
...
carbo_product = carbo %>%
                ...
                summarize(num_households = n_distinct(household), 
                  num_store_front = sum(display_desc == 'Store Front'),
                  num_in_aisle = sum(display_desc == 'In-Shelf'),
...
```
We can also visualze with a dendrogram, where the height of each branch is determined by linkage and distance decisions we made during clustering. The x-axis plots observations, so we see that using 3 clusters, one is very small only including a few observations (because it is narrow). The labels are hard to read as there are 900 observations.
