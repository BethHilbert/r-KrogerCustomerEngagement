...
### Plan
I will use the Carbo_Loading  
[dataset](http://uc-r.github.io/data_wrangling/final-project) provided by 84.51. 
...
analyze what differentiates the resulting clusters.
...
[data](http://uc-r.github.io/data_wrangling/final-project) 
...
Next I aggregated the product information, segmented these products into groups 
with hierarchical clustering model, and then analyzed what differentiates the 
resulting clusters. 
...
Observations are divided into meaningful groups that share common characteristics. 
...
is that I would like to contrast the clusters and there would be more meaningful 
information comparing 3 groups. 
...
from some weekly mailer feature categories (wrap front, back page, and interior 
line item). They also visited the fewest stores for the fewest number of weeks. 
This is not a group that is motivated to visit the store from weekly promotions.

Cluster 2 is the largest cluster and also appears to be the most influenced to 
purchase items in the weekly mailers. They buy in higher quantities and for the 
longest period (stretching from week 47 to week 100). Whereas the other two 
clusters averaged 0 and 1.1 sales resulting from wrap front, this group averaged 
70 from the wrap front and 37 from the wrap back. It would be worth looking further 
into what motivates this group in order to focus our promotion campaigns. 

Cluster 3 is between the other two in terms of influence by mailers and displays, 
but they purchase a higher percent of items with coupons. It's interesting that 
...
displays. It could be this group purchases what they came for but not much extra.
...
By clustering the products, we identified three distinct groups. An interesting 
insight was how the model defined the groups. A very large group was created that 
bought in higher quantities and responded to promotions. But one cluster appears 
to not be influences by promotions. Another cluster appears to not respond to 
mailer or display promotions but to use coupons in higher proportions. 

An implication is to look further into the cluster that uses coupons to see what 
types of products they are purchasing and why they are not responding to the 
other promotion types as frequently. 
